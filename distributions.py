...
class Distribution:
    ...
    def write_dist(self, dt, write_directory):
        """
        This function writes down the MW values of the CDF for each 0.025 interval.

        Args:
            dt: The day and hour for which to determine the values.
        """
        if self.dimension != 1:
            raise RuntimeError("The dimension should be set to 1, it is currently set to" + str(self.dimension))
        # kill function, if the plot directory has the same name.
        distr_dir = 'distributions'  # hardwired sub-directory name
        if globals.GC.plot_directory == distr_dir:
            raise RuntimeError("The plot directory should not have the same name as the distribution directory.")
        if write_directory == '':
            print("As there is no write directory given, the distributions will not be computed.")
            return
        i = 0
        # we need a dataframe to add onto, so we just use the first value for zero.
        cdf_dataframe = pd.DataFrame(index={str(dt)}, data={'power': [round(self.inverse_cdf(0), 4)], 'CDF value': [0]})
        while i < 0.975:
            i += 0.025
            power = self.inverse_cdf(i)
            temp_df = pd.DataFrame(index={str(dt)}, data={'power': [round(power, 4)], 'CDF value': [round(i, 3)]})
            cdf_dataframe = cdf_dataframe.append(temp_df)
        # the last value has to be done manually, because rounding causes and error as on the edge.
        temp_df = pd.DataFrame(index={str(dt)}, data={'power': [round(self.inverse_cdf(1), 4)], 'CDF value': [1]})
        cdf_dataframe = cdf_dataframe.append(temp_df)
        if not os.path.exists(write_directory + os.sep + distr_dir + os.sep):
            os.makedirs(write_directory + os.sep + distr_dir + os.sep)

        date = dt.strftime('%Y-%m-%d-%H%M')
        cdf_dataframe.to_csv(path_or_buf=write_directory + os.sep + distr_dir + os.sep + date + '.csv')
...
class MultivariateEmpiricalDistribution(Distribution):
    ...
    def plot_all_peels(self, dt, plot_directory):
        """
        This function plots all possible peels of the halfspace or direct quantile region function.
        This plotting will replace the usual plotting, when no alpha is provided.

        Args:
            dt: The datetime used for the plot. The datetime has no influence on the plotting,
            but is needed to give the plot a unique name.
            plot_directory: The directory, where the plots will be saved in.
        """
        if not os.path.exists(plot_directory):
            print("Creating plot directory=%s" % plot_directory)
            os.makedirs(plot_directory)
        if globals.GC.quantile_region_algorithm == "halfspace":
            if PYHULL_INSTALLED:
                _, hull, points_to_draw, _ = self.halfspacedepth_quantile_region(0)
                self.halfspacedepth_quantile_region(1)
            else:
                _, hull, points_to_draw = self.halfspacedepth_quantile_region2(0)
                self.halfspacedepth_quantile_region2(1)
        elif globals.GC.quantile_region_algorithm == "direct":
            _, hull, points_to_draw, _ = self.direct_convex_hull_quantile_region(0)
            self.direct_convex_hull_quantile_region(1)
        else:
            raise RuntimeError("plotting all peels only works for the 'halfspace' and 'direct' setting "
                               "for the option --quantile-region-algorithm.")

        # we will iterate thru all the possible axis combinations.
        x_axis = 0
        # x_axis iterates thru every dimension: 0, 1, 2, ...
        while x_axis < len(self.allhulls[0].tolist()[0]):
            y_axis = x_axis + 1
            # y_axis iterates thru dimensions bigger than x, because we need only one plot for (1,2) and (2,1).
            while y_axis < len(self.allhulls[0].tolist()[0]):
                # we make a new list of points, that have only the information for two dimensions.
                for points in self.allhulls:
                    if points is not None:
                        # lists are easier to shorten.
                        points_list = points.tolist()
                        two_dim_points = []
                        two_dim_points_to_draw = []
                        i = 0
                        # create a list of lists.
                        while i < len(points_list):
                            two_dim_points.append([])
                            i += 1
                        i = 0
                        while i < len(points_to_draw):
                            two_dim_points_to_draw.append([])
                            i += 1
                        i = 0
                        # add the two dimensional points to the list.
                        while i < len(points_list):
                            two_dim_points[i].append(points_list[i][x_axis])
                            two_dim_points[i].append(points_list[i][y_axis])
                            i += 1
                        i = 0
                        while i < len(points_to_draw):
                            two_dim_points_to_draw[i].append(points_to_draw[i][x_axis])
                            two_dim_points_to_draw[i].append(points_to_draw[i][y_axis])
                            i += 1
                        # numpy points are easier to plot than lists.
                        two_dim_points = np.array(two_dim_points)
                        nppoints_to_draw = np.array(two_dim_points_to_draw)
                        # twodhull is the convex hull for each projection.
                        twodhull = scipy.spatial.ConvexHull(two_dim_points)
                        # plotting starts here
                        plt.plot(nppoints_to_draw[:, 0], nppoints_to_draw[:, 1], 'o')
                        for simplex in twodhull.simplices:
                            plt.plot(two_dim_points[simplex, 0], two_dim_points[simplex, 1], 'k-')
                        date = dt.strftime('%Y-%m-%d-%H%M')
                        x_name = self._names[x_axis]
                        y_name = self._names[y_axis]

                        # axeslim = plt.gca()
                        # axeslim.set_xlim([-80, 40])
                        # axeslim.set_ylim([-80, 40])

                        plt.savefig(os.path.join(
                            plot_directory,
                            'convex_hull_figure_' + date + '(' + str(x_name) + ',' + str(y_name) + ').png'),
                            bbox_inches='tight')
                plt.clf()
                y_axis += 1
            x_axis += 1

    def plot_convex_hull(self, alpha, dt, plot_directory):
        """
        this function plots the convex hull and the points provided by the approximate_quantile_region function.
        For now 3- and higher-dimensional data will be projected down to two dimensions.
        ...
        Args:
            ...
            plot_directory: The directory, where the plots will be saved in.
        """
        # TODO: move this function outside the class
        # If the directory does not exist, create it
        if not os.path.exists(plot_directory):
            print("Creating plot directory=%s" % plot_directory)
            os.makedirs(plot_directory)
        if globals.GC.check_convex_hull is True:
            self.check_convex_hull()
        if alpha == 0:
            print("Warning: The alpha you submitted is equal to 0, which is invalid, and "
                  "therefor the convex hull of the whole data is being outputted instead.")

        if globals.GC.quantile_region_algorithm == 'mahal':
            points, hull, points_to_draw, _ = self.mahalanobis_quantile_region(alpha)
        elif globals.GC.quantile_region_algorithm == "halfspace":
            if PYHULL_INSTALLED:
                points, hull, points_to_draw, _ = self.halfspacedepth_quantile_region(alpha)
            else:
                points, hull, points_to_draw = self.halfspacedepth_quantile_region2(alpha)
        elif globals.GC.quantile_region_algorithm == "direct":
            points, hull, points_to_draw, _ = self.direct_convex_hull_quantile_region(alpha)
        else:
            raise RuntimeError(
                "The option --quantile-region-algorithm, must be set either to 'mahal', 'direct' or to 'halfspace'.")
        if points is not None:
            # we will iterate thru all the possible axis combinations.
            x_axis = 0
            # lists are easier to shorten.
            points_list = points.tolist()
            # x_axis iterates thru every dimension: 0, 1, 2, ...
            while x_axis < len(points.tolist()[0]):
                y_axis = x_axis + 1
                # y_axis iterates thru dimensions bigger than x, because we need only one plot for (1,2) and (2,1).
                while y_axis < len(points.tolist()[0]):
                    # we make a new list of points, that have only the information for two dimensions.
                    two_dim_points = []
                    two_dim_points_to_draw = []
                    i = 0
                    ...
                    # axeslim = plt.gca()
                    # axeslim.set_xlim([-60, 60])
                    # axeslim.set_ylim([-60, 60])

                    plt.savefig(os.path.join(
                        plot_directory, 'convex_hull_figure_' + date + '(' + str(x_name) + ',' + str(y_name) + ').png'),
                        bbox_inches='tight')
                    # we need to clear the graph before we can plot again.
                    plt.clf()
                    y_axis += 1
                x_axis += 1
